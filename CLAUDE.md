...
`plugins/multivendorx/docs/` and `plugins/notifima/docs/` are public, third-party-developer-facing references (hooks/filters, REST controller pattern, admin/frontend dashboard architecture, container/data-store/template-override/module-integration topics) — distinct from this file and from the parent repo's `.claude/rules/` (both internal AI guidance). Since these are base plugins, their docs describe several mechanisms at their source rather than just how their Pro counterpart extends them (e.g. `multivendorx`'s module loader itself and the actual REST namespace/dashboard mount points; `notifima`'s `WC_Deprecated_Hooks`-based legacy-hook shim) — read each plugin's own `docs/README.md` first if you're building against that plugin directly rather than a Pro add-on. `notifima`'s docs are considerably smaller in scope than `multivendorx`'s (no module system, one custom table, ~30 hooks total) — don't assume the two are structured identically beyond the top-level section names. Generated by the parent repo's `developer-docs` skill (`.claude/skills/developer-docs/SKILL.md`).
