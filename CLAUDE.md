...
`plugins/multivendorx/docs/` is a public, third-party-developer-facing reference (hooks/filters, REST controller pattern, admin/frontend dashboard architecture, container/data-store/template-override/analytics/module-integration topics) — distinct from this file and from the parent repo's `.claude/rules/` (both internal AI guidance). Since this is the base plugin, its docs describe several mechanisms at their source rather than just how Pro extends them (e.g. the module loader itself, the actual REST namespace and dashboard mount points) — read `plugins/multivendorx/docs/README.md` first if you're building against this plugin directly rather than a Pro add-on. Generated by the parent repo's `developer-docs` skill (`.claude/skills/developer-docs/SKILL.md`).
