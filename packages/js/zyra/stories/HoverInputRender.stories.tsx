...
export const TestHoverInputRender = () => {
    const demoHoverInputRenderProps = {
        ...
        renderStaticContent: ({ label, placeholder }: { label: string; placeholder?: string }) => (
            <div title={placeholder}>{label}</div>
        ),
        renderEditableContent: ({ label, onLabelChange, placeholder }: { label: string; onLabelChange: (newLabel: string) => void; placeholder?: string }) => (
            <input
            type="text"
            value={label}
