...
const ExpandablePanelGroup: React.FC< ExpandablePanelGroupProps > = ( {
    ...
    useEffect( () => {
        if ( ! isWizardMode && ! methods?.length ) {
            return;
        }
    ...
    useEffect( () => {
        ...
        const valueMethods: ExpandablePanelMethod[] = Object.entries(
            updated
        ).map( ( [ id, method ] ) => ( {
            id,
            ...( method as any ),
