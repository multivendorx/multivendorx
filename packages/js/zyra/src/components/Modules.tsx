...
const Modules: React.FC< ModuleProps > = ( {
    ...
    const filteredModules = modulesArray.modules.filter( ( item ) => {
        if ( 'type' in item && item.type === 'separator' ) {
            ...
            const hasModulesInCategory = modulesArray.modules.some(
                ( module ) => {
                    ...
                    if ( mod.parent_category !== separatorCategory )
                        return false;
                    if (
        ...
        if ( ! module.parent_category && selectedCategory !== 'All' )
            return false;
        // Apply category filter
        if (
            selectedCategory !== 'All' &&
            module.parent_category !== selectedCategory
        ) {
            return false;
        }
        // Apply status filter
        ...
        <div className="module-container">
            ...
            <div className="category-filter">
                <div className="module-status-filter">
                    ...
                    <button
                        className={ `filter-button ${
                            selectedFilter === 'Inactive' ? 'active' : ''
                        }` }
