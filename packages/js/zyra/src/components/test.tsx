...
export const MultiCheckBoxUI: React.FC<MultiCheckBoxProps> = (props) => {
    ...
    const deleteOption = (index: number) => {
        ...
        const selectedValues = props.value?.filter(v => 
            updatedOptions.some(opt => opt.value === v)
        ) ?? [];
    ...
    return (
        <div className={props.wrapperClass}>
            ...
            {localOptions.map((option, index) => {
                ...
                const optionId = `toggle-switch-${option.key || index}`;

                return (
                    <div key={option.key || index} className="toggle-checkbox-header">
                        {props.rightContent && (
                            <p
                        ...
                        {editIndex === index ? (
                            ...
                            // View mode - proper checkbox layout
                            <div className={props.inputInnerWrapperClass}>
                                <div className="checkbox-wrapper">
                                    <input
                                        className={props.inputClass || 'checkbox-input'}
                                        id={optionId}
                                    ...
                                    <label htmlFor={optionId} className="checkbox-label">
                                        ...
                                        {option.proSetting && !props.appLocalizer?.khali_dabba && (
                                        ...
                                        {!option.proSetting && option.moduleEnabled && 
                                         !props.modules.includes(option.moduleEnabled) && (
                                            <span className="admin-pro-tag module">
                                                <i className={`adminfont-${option.moduleEnabled}`}></i>
            ...
            {props.addNewBtn && (
                <div className="add-new-option">
                    {showNewInput ? (
                        ...
                        >
                            <i className="adminfont-plus"></i> {props.addNewBtn}
                        </div>
                    )}
                </div>
            )}
...
const MultiCheckBox: FieldComponent = {
    render: ({ field, value, onChange, canAccess, appLocalizer, modules, settings, onOptionsChange, onBlocked }) => {
        ...
        // Get options from settings first, fallback to field.options
