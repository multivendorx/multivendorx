...
class FrontEnd {
    ...
    public function __construct() {
        ...
        add_filter( 'notifima_display_product_lead_time', array( $this, 'display_product_lead_time' ), 10 );       
         ...
         add_action( 'template_redirect', array( $this, 'maybe_hide_outofstock_message' ));
    ...
    public function display_product_subscription_form( $product_obj = null ) {
        ...
        global $product;
        $discontinued = $product->get_meta( '_custom_discontinued' ); 
        if ( $discontinued === 'no' ) {
        ...
        {
           echo '<p class="stock discontinued">This product has been discontinued.</p>';
        }
    }
    ...
    public function maybe_hide_outofstock_message() {
    if ( ! is_product() ) {
        return;
    }

    add_filter( 'gettext', [ $this, 'hide_outofstock_message' ], 10, 3 );
}

public function hide_outofstock_message( $translated, $text, $domain ) {
