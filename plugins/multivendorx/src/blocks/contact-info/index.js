...
registerBlockType('multivendorx/contact-info', {
    ...
    edit: ({ attributes, setAttributes }) => {
        ...
        const TEMPLATE = [
            ...
            ['core/paragraph', { 
                content: '<label>Name <input type="text" name="name" class="input-text" required /></label>',
                className: ' woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide'
            }],
            ['core/paragraph', { 
                content: '<label>Email <input type="email" name="email" class="input-text" required /></label>',
                className: ' woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide'
            }],
            ['core/paragraph', { 
                content: '<label>Message <textarea name="message" class="input-text" rows="4" required></textarea></label>',
                className: ' woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide'
            }],
            ['core/button', { 
                text: 'Send Message',
        ...
        return (
            <>
                <InspectorControls>
                    ...
                    <PanelBody title={__('Google reCAPTCHA Settings')} initialOpen={false}>
                        <PanelRow>
                            <ToggleControl
                                ...
                                {googleRecaptchaType === 'v2' && (
                                    <PanelRow>
                                        <TextareaControl
                                            ...
                                            rows={3}
                                            help={__('Enter the reCAPTCHA v2 script code')}
                                        />
                                    </PanelRow>
                                )}
                                    ...
                                    <>
                                        <PanelRow>
                                            <TextControl
                                                ...
                                                help={__('Enter your reCAPTCHA v3 site key')}
                                            />
                                        </PanelRow>
                                        <PanelRow>
                                            <TextControl
                                                ...
                                                help={__('Enter your reCAPTCHA v3 secret key')}
                                            />
                                        </PanelRow>
                                    </>
                                )}
                    ...
                    <div className="contact-form-block-editor">
                        ...
                        {enableGoogleRecaptcha && (
                            <div className="recaptcha-preview">
                                <p><strong>{__('reCAPTCHA Enabled:')}</strong> {googleRecaptchaType === 'v2' ? 'v2' : 'v3'}</p>
                            </div>
                        )}
                    </div>
                </div>
        ...
        return (
            <div {...blockProps}>
                <form className="woocommerce-form woocommerce-form-login login">
                    ...
                    {enableGoogleRecaptcha && (
                        <div className="multivendorx-recaptcha-settings" style={{ display: 'none' }}>
                            <input 
                                type="hidden" 
                            ...
                            {googleRecaptchaType === 'v2' && recaptchaV2Scripts && (
                                <div 
                                    ...
                                    dangerouslySetInnerHTML={{ __html: recaptchaV2Scripts }}
                                />
                            )}
