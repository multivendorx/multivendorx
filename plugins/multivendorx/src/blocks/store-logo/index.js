    ...
    edit: ({ attributes, setAttributes }) => {
        ...
        return (
            <>
                ...
                <InspectorControls>
                    <PanelBody title={__('Dimensions', 'multivendorx')} initialOpen={true}>
                        <SelectControl
                            label={__('Aspect ratio', 'multivendorx')}
                            value={aspectRatio}
                            options={[
                                { label: __('Square - 1:1', 'multivendorx'), value: '1:1' },
                                { label: __('16:9', 'multivendorx'), value: '16:9' },
                                { label: __('4:3', 'multivendorx'), value: '4:3' },
                                { label: __('3:2', 'multivendorx'), value: '3:2' },
                                { label: __('Custom', 'multivendorx'), value: 'custom' },
                            ]}
                        ...
                        <NumberControl
                            ...
                            value={width}
                            onChange={(value) => setAttributes({ width: parseInt(value) || 100 })}
                            min={50}
                            max={500}
                            step={10}
                        />
                        ...
                        {aspectRatio === 'custom' && (
                            <NumberControl
                                ...
                                value={height}
                                onChange={(value) => setAttributes({ height: parseInt(value) || 100 })}
                                min={50}
                                max={500}
                                step={10}
                            />
                    ...
                    <PanelBody title={__('Color', 'multivendorx')} initialOpen={false}>
                        <div style={{ marginBottom: '20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                                <label>{__('Overlay', 'multivendorx')}</label>
                                {(overlayColor || customOverlayColor) && (
                                    <ColorIndicator colorValue={overlayColor || customOverlayColor} />
                                )}
                            </div>
                            <ColorPalette
